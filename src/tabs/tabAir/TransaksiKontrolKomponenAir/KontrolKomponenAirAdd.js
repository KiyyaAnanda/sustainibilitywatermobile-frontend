...
const KontrolKomponenAirAdd = () => {
  ...
  useEffect(() => {
    const fetchKomponenBocor = async () => {
      try {
        ...
        const data = await postUserArray("TransaksiKontrolKomponenAir/GetDataKomponenAirByBocor", filter);
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <FormLayout source={require("../../../assets/picturePng/EvaluasiTarget.png")}>
        <FormContainerWithBlur
          ...
          <DropDownForm
            ...
            onValueChange={async (value) => {
              ...
              const validationError = await validateInput("komponen", value, userSchema);
          ...
          <Input
            ...
            onChangeText={async (text) => {
              setJudul(text);
              const validationError = await validateInput("judul", text, userSchema);
          ...
          <Input
            ...
            onChangeText={async (text) => {
              setPic(text);
              const validationError = await validateInput("pic", text, userSchema);
          ...
          <DateInputPicker
            ...
            onChangeDate={async (formatted) => {
              ...
              const validationError = await validateInput("tglpengecekan", formatted, userSchema);
              setErrors((prev) => ({
                ...prev,
